...
Since these implementations are for binary outcome, we can expect that the overall Brier score may be inflated because the competing risks true contribution to the event of interest is ignored.
...
# Using individual censoring probability weights. 
# Extension of BinaryBrierScore
WeightedBinaryBrierScore(predictions=pred, 
            time = vdata$time, 
            status = vdata$status,
            cause = primary_event,
            tau = horizon, 
...
# Alternative implementation with the above formulas:
WeightedBinaryBrierScore2(predictions=pred,
                                tau=5,
                                time=vdata$time, 
                                status=vdata$status,
                                cause=primary_event, 
...
In order to consider the competing risks

```{r}
WeightedCRBrierScore(predictions=pred, 
            time = vdata$time, 
            status = vdata$status,
            cause = primary_event,
            tau = horizon,
...
The weighted.brier.score is the same as in riskRegression when considering censoring and competing risks. 


Simplification of Weighted brier:

The  simplification is based on the BrierScore.R function with censoring by adding an additional case with handling the competing risks:


```{r}
# Weighted version of brier score
# Valid for competing risks when censoring:
WeightedCRBrierScore2(predictions=pred, 
            time = vdata$time, 
            status = vdata$status,
            cause = primary_event,
            tau = horizon, 
...
```{r}
ScaledBrierScore2(predictions = pred,
                 predictions_null = pred_null,
                 tau=5,
                 time=vdata$time,
                 status=vdata$status,
...
censor.prob.KM.individual(time=vdata$time, status=vdata$status, cens.code=0)
