...
```{r}
# Models -------------------
fit_csh <- riskRegression::CSC(Hist(time, status_num) ~
age + size +
  ncat + hr_status,
data = rdata
)
...
Sine we want to move from the Score function in RiskRegression, we can have our own definition of Brier Score:
...
We obtain the same value as from the riskRegression::Score

(Next add CI)

##### Weighted Brier Score - IPCW
...
The original censoring is as follows:
...
The event is 187 and the right.censored is 103. This is a problem of not considering competing risks with prodlim or with RiskRegression. The original status == 1 becomes the right.censored, while the status == 2 becomes the event... so that is completly wrong by using these default methods.
...
# method = marginal when non-parametric kaplan meier
test <- ipcw(formula=Surv(time,censor.status)~1,
     data=vdata, 
...
**However if status is 0,1,2... when reversing, we have a NA for the status == 2, and therefore this method does not automatically work with competing risks, unless status is manually modifies to be binary (0:censor, 1:any event).**
...
I think it does not allow for the status == 2 (therefore converted into NA)
...
However, I think the correct is the following, when the reverse is TRUE to estimate the censoring probabilities:
...
In the paper, they generate samples and calculate the metrics on the samples. 
...
In the paper the IPA is the Scaled Brier.... I am not sure if they do it weighted.

Next step: Check if the Score implementation is using ipcw or not for the Scaled Brier. 

#### Inegrated brier score. 
