#'
#' Title: Calibration Numerical Measures
#' 
...
#'
#' @return List of each of the numerical measures at time tau.


CalibrationNumericalMeasures <- function(predictions, # any model CIF or S predictions
  ...
  values <- CalibrationPlot(predictions=predictions, data=data, time=time, status=status, tau=tau, 
                             cause=cause, cens.code=cens.code, predictions.type=predictions.type, 
                             method=method, quantiles=quantiles, bandwidth, 
                             n.knots=n.knots, graph = FALSE)
  
  # Calculate the error
  error <- values$pred - values$obs

  # Compute the numerical measures
  measures <- c(
    "ICI" = mean(abs(error)), 
    setNames(quantile(abs(error), c(0.5, 0.9)), c("E50", "E90")),
    "Emax" = max(abs(error)),
    "Root squared bias" = sqrt(mean(error^2))
  )
  
  return(result = list(tau=tau, values=values, error=error, calibr.measures=measures))
}
